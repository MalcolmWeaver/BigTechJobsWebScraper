    ...
    def parse_get_job_listings(self, response) -> JobPosting:
        ...
        try:
            ...
            for job_data in raw_jobs:
                filtered_data = {
                    k: v for k, v in job_data.items() if k in job_model_fields
                }
                filtered_data["company"] = "meta"
                filtered_data["posting_url"] = (
                    f"https://www.metacareers.com/jobs/{job_data['id']}"
                )
    ...
    def parse_get_job_with_details(
        ...
        if script_tag:
            ...
            mapped_data = {
                ...
                "responsibilities": (
                    data.get("responsibilities", "").split(";")
                    if data.get("responsibilities")
                    else None
                ),
                "requirements": (
                    data.get("qualifications", "").split(";")
                ...
                "extra_qualifications": (
                    [
                        ...
                        if "qualification" in key.lower() and val
                        for qual in val.split(";")
                    ]
                    if any("qualification" in key.lower() for key in data.keys())
                    else None
                ),
                "posted_date": (
                    datetime.fromisoformat(data["datePosted"])
